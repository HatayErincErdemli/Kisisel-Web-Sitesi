...
function Profile() {
  return (
    <div className="w-auto h-auto flex flex-col items-start px-60 pt-10 pb-20 gap-8 bg-[#4731d2]">
      <h2 className="text-4xl font-bold pt-10">{languages.en.profile.title}</h2>

      <div className="flex flex-row justify-between w-full h-full">
        <div className="flex flex-col gap-8 w-full h-full">
          <p className="text-3xl font-semibold">{languages.en.profile.info}</p>
          <div className="">
            {languages.en.profile.info_items.map(({ title, answer }, index) => {
              return (
                ...
                >
                  ...
                  <p className="text-lg text-right">{answer}</p>
                </div>
              );
            })}
          </div>
